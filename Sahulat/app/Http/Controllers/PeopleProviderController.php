...
class PeopleProviderController extends Controller
{
  ...
  {
    $services = DB::table('service_providers')->join('services', 'service_providers.service_id', '=', 'services.id')->orderBy('rating','desc')->take(10)->get();
    //dd($services);
  ...
  function deleteServiceAdmin($serviceid){
    if (strtoupper(auth()->user()->role)=="ADMIN"){
      //dd($serviceid);
      if($serviceid){
